...
function gauge<T>(lines: string[], name: string, help: string, entries: T[], emit: (entry: T) => string | string[]) {
...
export function formatPrometheus(
  ...
  gauge(
    lines,
    "fitbit_sleep_hours_asleep",
    "Total hours asleep.",
    mainSleeps,
    (s) => `fitbit_sleep_hours_asleep{date="${s.dateOfSleep}"} ${(s.minutesAsleep / 60).toFixed(2)}`,
  );

  gauge(
    lines,
    "fitbit_sleep_hours_in_bed",
    "Total hours in bed.",
    mainSleeps,
    (s) => `fitbit_sleep_hours_in_bed{date="${s.dateOfSleep}"} ${(s.timeInBed / 60).toFixed(2)}`,
  );
  ...
  gauge(
    ...
    mainSleeps,
    (s) => `fitbit_sleep_efficiency{date="${s.dateOfSleep}"} ${s.efficiency}`,
  );
  ...
  gauge(
    lines,
    "fitbit_hrv_daily_rmssd",
    "Daily RMSSD heart rate variability (ms).",
    hrvData.hrv,
    (entry) => `fitbit_hrv_daily_rmssd{date="${entry.dateTime}"} ${entry.value.dailyRmssd.toFixed(3)}`,
  );

  gauge(
    lines,
    "fitbit_hrv_deep_rmssd",
    "Deep-sleep RMSSD heart rate variability (ms).",
    hrvData.hrv,
    (entry) => `fitbit_hrv_deep_rmssd{date="${entry.dateTime}"} ${entry.value.deepRmssd.toFixed(3)}`,
  );
  ...
  gauge(
    ...
    spo2Data,
    (entry) => `fitbit_spo2_avg{date="${entry.dateTime}"} ${entry.value.avg}`,
  );
  ...
  gauge(
    ...
    spo2Data,
    (entry) => `fitbit_spo2_min{date="${entry.dateTime}"} ${entry.value.min}`,
  );
  ...
  gauge(
    ...
    spo2Data,
    (entry) => `fitbit_spo2_max{date="${entry.dateTime}"} ${entry.value.max}`,
  );
  ...
  gauge(
    ...
    brData.br,
    (entry) => `fitbit_breathing_rate{date="${entry.dateTime}"} ${entry.value.breathingRate}`,
  );
  ...
  gauge(
    ...
    (entry) => `fitbit_steps{date="${entry.dateTime}"} ${entry.value}`,
  );
  ...
  gauge(
    ...
    caloriesData["activities-calories"],
    (entry) => `fitbit_calories{date="${entry.dateTime}"} ${entry.value}`,
  );
  ...
  gauge(
    ...
    distanceData["activities-distance"],
    (entry) => `fitbit_distance{date="${entry.dateTime}"} ${entry.value}`,
  );
  ...
  gauge(
    ...
    distanceData["activities-distance"],
    (entry) => `fitbit_distance_mi{date="${entry.dateTime}"} ${(parseFloat(entry.value) * 0.621371).toFixed(2)}`,
  );
  ...
  gauge(
    ...
    floorsData["activities-floors"],
    (entry) => `fitbit_floors{date="${entry.dateTime}"} ${entry.value}`,
  );
